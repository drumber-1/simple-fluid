...
void set_bounds_wall(FluidVariable var, FluidArray<NX, NY> array) {
    for (size_t i = 1; i < NX - 1; ++i) {
        array(i, 0     ) = (var == FluidVariable::VELOCITYY) ? -array(i, 1     ) : array(i, 1     );
        array(i, NY - 1) = (var == FluidVariable::VELOCITYY) ? -array(i, NY - 2) : array(i, NY - 2);
    }
    for (size_t j = 1; j < NY - 1; ++j) {
        array(0     , j) = var == (FluidVariable::VELOCITYX) ? -array(0     , j) : array(0     , j);
        array(NX - 1, j) = var == (FluidVariable::VELOCITYX) ? -array(NX - 2, j) : array(NX - 2, j);
    }
